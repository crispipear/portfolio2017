...
function introType(){
  var str = 'UX Designer & Front End Developer';
  var typing = ' ';
  var i = 0;
  (function type() {
    if (i < str.length) {
      typing = typing + str.charAt(i);
      $('#landpg p').text(typing + '_');
    } else {
      $('#landpg p').text(typing + '_');
      return false;
    }
    var ran = Math.floor(Math.random() * 100) + 50; //this gives a random delay between each character so it's more realistic
